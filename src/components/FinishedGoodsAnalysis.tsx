...
export default function FinishedGoodsAnalysis({
  ...
  const getProductValueMultiplier = (p: Product, mode: UnitOption): number => {
    if (mode === 'cartons') {
      const divisor = (p.pcs_per_bag || 10) * (p.bags_per_carton || 4);
      return 1 / (divisor || 1);
    }
    if (mode === 'value') {
    ...
    return (
      <React.Fragment key={node.id}>
        <tr className={`hover:bg-slate-50 transition-colors ${depth === 0 ? 'bg-slate-50/50 font-semibold' : depth === 1 ? 'bg-slate-50/20' : ''}`}>
          ...
          <td className="px-4 py-3 text-right font-mono text-xs font-bold text-slate-900">${Math.round(node.metrics.salesValue).toLocaleString()}</td>
        </tr>
  ...
  return (
    <div className="space-y-6" id="finished_goods_psi_screen">
      <ContentHeader
        title="PSI Analysis"
        subtitle="Pivot-style balanced planning matrix tracking rolling Stock, Master Production Schedules, and Channel Demand Forecasts."
        actions={
          <button 
            onClick={loadAllData} 
                ...
                <table className="min-w-full divide-y divide-gray-200 text-left text-xs font-sans">
                  ...
                  <tfoot className="bg-slate-100 font-bold border-t-2 border-slate-300">
                    <tr>
                      ...
                      <td className="px-4 py-3 text-right font-mono text-slate-900">${Math.round(globalTotalMetrics.salesValue).toLocaleString()}</td>
                    </tr>
                  </tfoot>
                </table>
              </ScrollableTable>
                  ...
                  return (
                    <div key={i} className="space-y-1 text-xs">
                      <div className="flex justify-between items-center text-slate-600">
                        ...
                        <span className="font-mono font-bold">${Math.round(d.value).toLocaleString()}</span>
                      </div>
            ...
            <div className="p-6 overflow-y-auto space-y-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-xs">
                ...
                <div className="bg-slate-50 p-3 rounded-lg border border-slate-100">
                  ...
                  <span className="font-bold text-emerald-600 font-mono">${selectedSkuProduct.selling_price.toFixed(2)}</span>
                </div>
              </div>
