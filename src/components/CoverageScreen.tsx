...
export default function CoverageScreen({
  ...
  async function loadData() {
    ...
    try {
      ...
      console.error("Failed to fetch inventory coverage details", e);
    } finally {
      setLoading(false);
    }
  }
                ...
                <table className="min-w-full text-left text-[11px] border-collapse">
                  <thead className="bg-slate-50 text-[9px] font-bold text-slate-500 uppercase tracking-wider border-b border-slate-200">
                    <tr>
                      ...
                      <th className="px-3 py-2 border-r border-slate-200">Material Name</th>
                    ...
                    {filteredMaterials.map(m => {
                      ...
                      return (
                        <tr key={m.material_id} className="hover:bg-slate-50 transition-colors">
                          <td className="px-3 py-2 font-mono text-slate-500 font-semibold border-r border-slate-200">{m.sku}</td>
                          <td className="px-3 py-2 font-bold text-slate-800 border-r border-slate-200">{m.material_name}</td>
                          <td className="px-3 py-2 text-slate-500 border-r border-slate-200">{m.category_name}</td>
                          <td className="px-3 py-2 text-right font-semibold font-mono border-r border-slate-200">{m.opening_stock.toLocaleString()}</td>
                          <td className="px-3 py-2 text-right font-mono text-slate-500 border-r border-slate-200">{m.monthly_demand.toLocaleString()}</td>
              ...
              <ScrollableTable>
                <table className="min-w-full text-left text-[11px] border-collapse">
                  <thead className="bg-slate-50 text-[9px] font-bold text-slate-500 uppercase tracking-wider border-b border-slate-200">
                    <tr>
                      <th className="px-3 py-2 border-r border-slate-200">SKU</th>
                      <th className="px-3 py-2 border-r border-slate-200">Product Description</th>
                    ...
                    {filteredProducts.map(p => {
                      ...
                      return (
                        <tr key={p.product_id} className="hover:bg-slate-50 transition-colors">
                          <td className="px-3 py-2 font-mono text-slate-500 font-semibold border-r border-slate-200">{p.sku}</td>
                          <td className="px-3 py-2 font-bold text-slate-800 border-r border-slate-200">{p.product_name}</td>
                          <td className="px-3 py-2 border-r border-slate-200">
                            <span className="px-1.5 py-0.5 bg-slate-100 text-slate-700 text-[9px] font-semibold uppercase font-mono rounded-[3px]">
                              {p.product_line}
                            </span>
                          </td>
                          <td className="px-3 py-2 text-right font-mono text-slate-600 border-r border-slate-200">{p.opening_stock.toLocaleString()}</td>
                          <td className="px-3 py-2 text-right font-mono text-slate-400 border-r border-slate-200">{p.monthly_demand.toLocaleString()}</td>
                          <td className={`px-3 py-2 text-right font-bold ${rowColor}`}>
                            <span className={`px-1.5 py-0.5 rounded-[3px] font-mono text-[10px] font-bold border ${
